...
{	
    ...
    public function afterAddressSave(Varien_Event_Observer $observer)
    {
    	...
    	if($customer_address->hasDataChanged() || !Epoint_SwissPost_Customer_Model_Odoo::addressIsConnected($customer_address)){
    ...
    function SwissPostApiBeforeCreateOrder(Varien_Event_Observer $observer){
        ...
        $connection = Mage::getModel('swisspost_customer/odoo')->loadByAddress($address);
			  $account = Mage::helper('swisspost_customer')->__toSwissPostFromAddress($address, $order, $mapping = 'order_account_anonymous');
     ...
     *
     */
    function SwissPostApiAfterCreateUpdateAccount(Varien_Event_Observer $observer){
    	// Load customer.
        $customer = $observer->getCustomer();
     ...
     *
     */
    function SwissPostApiAfterCreateUpdateAnonymousAccount(Varien_Event_Observer $observer){
    	// Load customer.
