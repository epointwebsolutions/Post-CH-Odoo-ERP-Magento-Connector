...
class Epoint_SwissPost_Customer_Helper_Data extends Mage_Core_Helper_Abstract {
  ...
  public function __toSwissPost(Mage_Customer_Model_Customer $customer , $mapping = 'customer'){
    ...
    if(!isset($account_values['account_gender'])){
      $account_values['account_gender'] = $customer->getGender() == 1 ? 'male' : ($customer->getGender() == 2 ? 'female' : 'other');
    }
    ...
    if((int)$address_id){
    	...
    	//account_street_no
    	/*
    	$account_values['account_street_no'] = '';
    	$account_values['account_po_box'] = '';
    	$account_values['account_mypost'] = '';
    	*/
    	...
    	if(isset($account_values['account_zip'])){
			$account_values['account_zip'] = Epoint_SwissPost_Api_Helper_Address::fixZipCode($account_values['account_zip'], $account_values['account_country']);
		  }
   ...
   *
   */
  public function __toSwissPostFromAddress(Mage_Core_Model_Abstract $address, Mage_Core_Model_Abstract $order, $mapping = 'anonymous_customer'){
	 // Load mapping
    ...
    if(!($account_values['account_gender'])){
      $account_values['account_gender'] = $address->getGender() == 1 ? 'male' : ($address->getGender() == 2 ? 'female' : 'other');
    }
  ...
  public static function __toSwissPostGroup($customerId){
  	static $config;
  	if(!isset($config)){
  		$config = Mage::helper('swisspost_api')->textToArray(Mage::getStoreConfig(self::XML_CONFIG_PATH_GROUP_MAPPING));
  	}
  	//Default group name
