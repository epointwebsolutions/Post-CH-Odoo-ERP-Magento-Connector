...
/**
  ...
  public function __toSwissPost(Mage_Customer_Model_Customer $customer, Mage_Core_Model_Abstract $customerAddress){
    ...
    if(!isset($address_values['address_gender'])){
      $address_values['address_gender'] = $customerAddress->getGender() == 1 ? 'male' : ($customerAddress->getGender() == 2 ? 'female' : 'other');
    }
    ...
    if(!isset($address_values['address_website'])){
      $address_values['address_website'] = $customer->getWebsiteId();
    }*/
    if(!isset($address_values['address_maintag'])){
  	...
  	//address_street_no
  	/*
  	$address_values['address_street_no'] = '';
  	$address_values['address_po_box'] = '';
  	$address_values['address_mypost'] = '';
  	*/
  	...
  	if(isset($address_values['address_zip'])){
  		$address_values['address_zip'] = Epoint_SwissPost_Api_Helper_Address::fixZipCode($address_values['address_zip'], $address_values['address_country']);
  	}
