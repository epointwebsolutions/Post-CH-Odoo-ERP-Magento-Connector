...
$setup->addAttribute(
    'customer_address', 'odoo_id', array(
        'input'      => 'text',
        'type'       => 'int',
        'label'      => 'Odoo Id',
        'required'   => 0,
...
$table = $installer->getConnection()
    ...
    ->addColumn(
        'mail', Varien_Db_Ddl_Table::TYPE_VARCHAR, 128, array(
        'nullable' => false,
        'default'  => '',
    ), 'Email customer'
    )
    ->addColumn(
        'odoo_id', Varien_Db_Ddl_Table::TYPE_INTEGER, 11, array(
        'nullable' => false,
        'unsigned' => true,
        'default'  => '0',
    ), 'External odoo_id'
    ...
    ->addColumn(
        'customer_id', Varien_Db_Ddl_Table::TYPE_INTEGER, 11, array(
        'nullable' => false,
        'unsigned' => true,
        'default'  => '0',
    ), 'Customer id'
...
if ($installer->tableExists($installer->getTable('swisspost_customer/odoo'))) {
    $installer->getConnection()
        ->addIndex(
            ...
            $installer->getIdxName(
                'swisspost_customer/odoo', array('mail'), Varien_Db_Adapter_Interface::INDEX_TYPE_UNIQUE
            ),
            array('mail'),
    ...
    $installer->getConnection()
        ->addIndex(
            $installer->getTable('swisspost_customer/odoo'),
            $installer->getIdxName(
                'swisspost_customer/odoo', array('odoo_id'), Varien_Db_Adapter_Interface::INDEX_TYPE_INDEX
            ),
            array('odoo_id'),
            Varien_Db_Adapter_Interface::INDEX_TYPE_INDEX
        );
    $installer->getConnection()
        ->addIndex(
            $installer->getTable('swisspost_customer/odoo'),
            $installer->getIdxName(
                'swisspost_customer/odoo', array('customer_id'), Varien_Db_Adapter_Interface::INDEX_TYPE_INDEX
            ),
            array('customer_id'),
            Varien_Db_Adapter_Interface::INDEX_TYPE_INDEX
        );
