...
class Epoint_SwissPost_Catalog_Helper_Data extends Mage_Core_Helper_Abstract {
  ...
  public static function __toTaxClassId($odoo_tax_class_id){
    ...
    $configured = Mage::helper('swisspost_api')->textToArray(Mage::getStoreConfig(self::XML_CONFIG_PATH_TAX_CLASS_MAPPING));
    $mapping = array();
    if($configured){
  ...
  public static function __fromDynamicAttributes($item){
  	...
  	if(!isset($mapping)){
    	$mapping = Mage::helper('swisspost_api')->textToArray(Mage::getStoreConfig(self::XML_CONFIG_PATH_DINAMYC_ATTRIBUTE_MAPPING));
  	}
  	$odoo_values = array();
