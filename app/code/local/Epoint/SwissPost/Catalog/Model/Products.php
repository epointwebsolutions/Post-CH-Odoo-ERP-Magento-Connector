...
class Epoint_SwissPost_Catalog_Model_Products extends Mage_Core_Model_Abstract {
  ...
  public function import($items = array()){
    foreach($items as $_item){
      ...
      $imageAttached = 0;
      if($product->getId() && Mage::getStoreConfig(Epoint_SwissPost_Catalog_Helper_Data::XML_CONFIG_ENABLE_IMPORT_IMAGES)){
      ...
      try{
        ...
        if(!$imageAttached && $product->getId() && Mage::getStoreConfig(Epoint_SwissPost_Catalog_Helper_Data::XML_CONFIG_ENABLE_IMPORT_IMAGES)){
          $product = $this->attachImage($product, $_item);
   ...
   * @param $items
   */
  function updateInventory($items = array()){
    foreach($items as $_item){
      ...
      try{
        ...
        Mage::helper('swisspost_api')->log("ERROR: while processing product inventory with product_code=$product_code");
        Mage::helper('swisspost_api')->logException($e);
      }
    }
  }
  ...
  // Apply mapping, from magento attribute code, 
	...
	$default_values = Mage::helper('swisspost_api')->extractDefaultValues(Epoint_SwissPost_Catalog_Helper_Data::XML_CONFIG_PATH_DEFAULT_VALUES);
   ...
   * @param string $item_language_code
  ...
  protected function _saveImage($image, $type) {
    ...
    if($type == 'binary'){
      ...
      $file_name           = md5($image . strtotime('now')).'.'.$image_type; //give a new name, you can modify as per your requirement
  ...
  public static function attachImage($product){
    ...
    foreach($files as $filepath) {
      ...
      switch ($can_be_imported){
        case -11:
          Mage::helper('swisspost_api')->log(Mage::helper('core')->__('Image exists %s for sku: %s', $filepath, $sku));
          continue 2;
          break;
        case -1:
          Mage::helper('swisspost_api')->log(Mage::helper('core')->__('Image moved %s for sku: %s', $filepath, $sku));
          continue 2;
        case -4:
          Mage::helper('swisspost_api')->log(Mage::helper('core')->__('Image exists, found using md5 method %s for sku: %s', $filepath, $sku));
          continue 2;
          break;
      }
      if(self::imageIsBroken($filepath)){
        Mage::helper('swisspost_api')->log(Mage::helper('core')->__('Image cannot be imported %s, is broken sku: %s', $filepath, $sku));
        continue;
      }
      if(filesize($filepath)) {
        ...
        try {
          ...
          Mage::helper('swisspost_api')->log(Mage::helper('core')->__('Attach image on product: %s, sku: %s', $filepath, $sku));
        }catch (Exception $e) {
          Mage::helper('swisspost_api')->log(Mage::helper('core')->__('Error on save image: %s, sku: %s, message: %s', $filepath, $sku, $e->getMessage()));
        }
      } else {
        Mage::helper('swisspost_api')->log(Mage::helper('core')->__('Error on save image: %s, sku: %s, message: File size is 0', $filepath, $sku));
      }
    }
    return $product;
  }
  ...
  private static function _canBeImported($product, $filepath){
    ...
    if(!isset($product_items[$productId])){
      ...
      foreach($product_items[$productId]['items'] as $k=>$item){
        ...
        $product_items[$productId]['path'][$info['basename']] = Mage::getBaseDir('media').DS.'catalog'.DS.'product'.$item['file'];
        $product_items[$productId]['size'][$info['basename']] = filesize($product_items[$productId]['path'][$info['basename']]);
        $product_items[$productId]['md5'][$info['basename']] = md5(file_get_contents($product_items[$productId]['path'][$info['basename']]));
      }
    }
   ...
   *
   * @param unknown_type $filepath
   * @param unknown_type $mage_file_path
   */
  private static function _moveFile($product, $filepath, $mage_file_path){
    try {
      ...
      Mage::helper('swisspost_api')->log(Mage::helper('core')->__('Move image content: %s to %s, sku: %s', $filepath, $mage_file_path, $product->getSku()));
    }catch (Exception $e){
      Mage::helper('swisspost_api')->log(Mage::helper('core')->__('Error on move content image: %s, sku: %s, message: %s', $filepath, $product->getSku(), $e->getMessage()));
    }
  }
  /**
   * check if file was uploaded in file
   *
   * @param Ex: 00990270099027.jpg  $file
   * @param array $files 00990270099027_1_24_1.jpg
   */
